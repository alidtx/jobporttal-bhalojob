        ...
        if (isset($user) && count($profileEducation)):
            foreach ($profileEducation as $education):
                ...
                $html .= '<div class="panel panel-info" id="education_' . $education->id . '">
						  ...
						  <p class="text-left"><h5>ENTRANCE YEAR: ' . $education->entrance_year . '</h5></p>
						  
						  <p class="text-left">' . $education->institution_name . ' | ' . $education->getCity('city') . '</p>
						  <p class="text-left">' . $education->major . '</p>
						  </div>
						<div class="panel-footer"><a href="javascript:void(0);" onclick="showProfileEducationEditModal(' . $education->id . ',' . $education->degree_type_id . ');" class="text text-default">' . __('Edit') . '</a>&nbsp;|&nbsp;<a href="javascript:void(0);" onclick="delete_profile_education(' . $education->id . ');" class="text text-danger">' . __('Delete') . '</a></div>
						</div>';
            endforeach;
        endif;
